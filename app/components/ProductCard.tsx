...
export default function ProductCard({
    ...
    if (variant === "horizontal") {
        return (
            <div className="flex flex-row items-center !bg-white !rounded-xl !border !border-gray-100 !overflow-hidden group !h-auto relative">
                {badge && (
                    <span className={`product-badge ${badge === "sale" ? "badge-sale" : "badge-new"} !absolute !top-2 !left-2 !z-10`}>
                        {badge === "sale" ? "Sale" : "New"}
                    </span>
                )}
                ...
                <div className="p-5 flex-1 flex flex-col justify-center min-w-0 h-full">
                    <div>
                        <div className="flex items-center justify-between gap-2 mb-1.5">
                            <span className="!text-[9px] !bg-white !text-gray-400 !border !border-gray-100 !px-2 !py-0.5 !rounded !uppercase !font-bold tracking-tight">{category}</span>
                    ...
                    <div className="mt-4">
                        <div className="flex items-center gap-2 mb-3">
                            <span className="!text-[17px] !font-black !text-gray-900">${price.toFixed(2)}</span>
                            {originalPrice && (
                                <span className="!text-[10px] !font-medium !text-gray-300 !line-through">${originalPrice.toFixed(2)}</span>
                            )}
                        </div>
    ...
    // Default Vertical Variant (Home Page) - Restored from user's provided code
    return (
